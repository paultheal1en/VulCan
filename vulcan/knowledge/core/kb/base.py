...
class KBService(ABC):
    ...
    def search_docs(
        self, query: str, top_k: int, score_threshold: float, context_window: int
    ) -> List[Document]:
        """
...
class KBServiceFactory:
    ...
    def get_service(
        ...
        if SupportedVSType.MILVUS == vector_store_type:
            from vulcan.knowledge.core.kb.service.milvus_kb_service import (
                MilvusKBService,
            )
...
def get_available_vs_types() -> List[str]:
    ...
    """
    return [
        getattr(SupportedVSType, attr)
        for attr in dir(SupportedVSType)
