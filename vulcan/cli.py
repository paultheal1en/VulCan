...
@main.command(
    "init",
    help="Initialize the project: create directories, database tables, and config files.",
)
def init():
    ...
    try:
        ...
        click.secho(
            f"❌ Error initializing database. Please check your db_config.yaml. Error: {e}",
            fg="red",
        )
        return
    ...
    try:
        if not (project_root / "config.yaml").exists():
            ...
            click.secho(
                "✅ Generated default configuration files successfully.", fg="green"
            )
        else:
            click.secho(
                "ℹ️ Configuration files already exist, skipping generation.", fg="yellow"
            )
    except Exception as e:
