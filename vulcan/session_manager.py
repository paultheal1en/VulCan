...
from .repository.session_repository import update_session_in_db
console = Console()


def load_or_create_session(initial_mission_prompt: Optional[str] = None) -> Optional[Session]:
    """
    ...
    if confirm("Bạn có muốn tiếp tục từ một phiên làm việc trước đó không?"):
        ...
        if not sessions:
            console.print("[yellow]Không tìm thấy phiên làm việc nào. Bắt đầu một phiên mới.[/yellow]")
...
def update_current_planner_id(session_id: str, planner_id: str):
    """Finds a session by ID and updates its current_planner_id."""
    from .db_session import session_scope
    with session_scope() as db_session:
        session_model = db_session.query(SessionModel).filter(SessionModel.id == session_id).first()
        if session_model:
            session_model.current_planner_id = planner_id
            db_session.commit()